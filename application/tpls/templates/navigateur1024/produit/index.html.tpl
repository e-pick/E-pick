		...
		<div id="conteneur_menu">
			...
			<form method="post" action="" class="form search">
				<fieldset>   
					...
					<label for="zone">{$txt_zone}</label>
						...
						{else}
							<select name="zoneFilter" id="zone" value="" disabled></select>
						{/if}
					<br style="clear:both" />
					<label for="rayon">{$txt_rayon}</label>
						...
						{else}
							<select name="rayonFilter" id="rayon" value="" disabled></select>
						{/if}
					<br style="clear:both" />
					{if $finesse_utilisee >= $finesse_segment}
						<label for="segment">{$txt_segment}</label>
							...
							{else}
								<select name="segmentFilter" id="segment" value="" disabled></select>
							{/if}
						<br style="clear:both" />
						{if $finesse_utilisee >= $finesse_etagere}
							<label for="etagere">{$txt_etagere}</label>
								...
								{else}
									<select name="etagereFilter" id="etagere" value="" disabled></select>
								{/if}
							<br style="clear:both" />
						{/if}
					{/if}
						
				<div class="reset"><a href="{$application_path}produit" title="{$txt_effacer}">{$txt_effacer}</a></div>
			...
			<table class="tableau ligne" id="sorter">
				<tr>
					...
					<th style="width:30%;"><img src="{$application_path}images/sort.gif" alt="sort"  /> {$txt_libelle_produit}</th>
