	...
	<div>	
		...
		<div id="conteneur_menu">
			...
			<form method="post" action="" class="form search">
				...
				<div class="reset"><a href="{$application_path}zone/affecter" title="{$txt_effacer}">{$txt_effacer}</a></div>
			...
			<form method="post" action="">
				<div style="width:300px;margin:auto">
						<fieldset>
							<legend> {$txt_affectZone}</legend>
							<p>{$txt_saisir_etage}</p>
							<label for="choixZone">{$txt_zone} </label>
							<select name="choixZone" id="choixZone" style="width:150px">
								<option value=""></option>
								{foreach from=$arrayZones key=id item=zone}
									<option value="{$zone->getIdzone()}">{$zone->getLibelle()}</option>
								{foreachelse}
									<option> </option>
								{/foreach}
							</select>
							<input type="submit" name="submitAffecter" id="submitAffecter" value="{$txt_affecter}"/>
						</fieldset>
				
				</div>
				<table class="tableau ligne" id="sorter">
					<tr>
						<th class="nosort" style="width:20px;"><input id="selectAll" type="checkbox" value="all"></th>
						<th style="width:40%;"><img src="{$application_path}images/sort.gif" alt="sort"  /> {$txt_libelle_rayon}</th>
						<th style="width:25%;"><img src="{$application_path}images/sort.gif" alt="sort" /> {$txt_etage}</th>			
						<th style="width:25%;"><img src="{$application_path}images/sort.gif" alt="sort" /> {$txt_zone}</th>						 
					</tr>
					{foreach from=$arrayRayons key=myId item=rayon}
						<tr>
							<td><input type="checkbox" class ="checkRayon" name="Rayon[]" value="{$rayon[0]->getIdrayon()}" {if isset($form_checkRayon) && !empty($form_checkRayon) && in_array({$rayon[0]->getIdrayon()},$form_checkRayon)}checked="true"{/if}> </td>
							<td><a href="{$application_path}rayon/afficher/{$rayon[0]->getIdrayon()}/0/0" title="{$txt_afficher_rayon}">{$rayon[0]->getLibelle()}</a></td>
