{include file="$user_template/common/page_header_lite.tpl"}
	...
	<div id="conteneur">
		...
		<div class="etage_demo" id="{$currentEtage->getIdetage()}" style="width:{$currentEtage->getlargeur()}px;height:{$currentEtage->gethauteur()}px;margin:auto;"> 
			...
			{foreach from=$arrayObstacles key=myId item=i}  
				...
				{else}
					<div class="obstacle_demo" id="{$i->getIdObstacle()}" style="position:absolute;top:{$i->getPosition_top()}px;left:{$i->getPosition_left()}px;width:{$i->getLargeur()}px;height:{$i->getHauteur()}px;background-color:#{$i->getCouleur()}"></div>
				{/if}
			{/foreach}
		...
		<div>
			 <!-- tracage des traits  -->
			 <textarea  id="point_res" style="display:none;"  rows="10" cols="25">
			 {foreach from=$listeDePointsARelier key=myId item=i}{$i[0]},{$i[1]},{$i[2]};{/foreach}</textarea> 
		</div>
