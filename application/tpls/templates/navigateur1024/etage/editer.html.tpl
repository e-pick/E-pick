	...
	<div>	
		<div id="conteneur_menu">
			...
			<div class="lienhaut">
				...
				{if $form_idEtage != 1}<a href="{$application_path}etage/supprimer/{$form_idEtage}" title="" onclick="return confirm('{$txt_confirmSuppression} ?')">{$txt_lienSupprimer}</a> |{/if}
				<a href="javascript:history.go(-1)">{$txt_retour}</a></div>
			</div>
