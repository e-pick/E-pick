...
{include file="$user_template/common/page_header.tpl"}
					...
					<div id="legende" style="">
						...
						<div style="text-align:center;float:right;width:60%;">
							...
							<a href="#" onclick="return false;"  id="ajouter_obstacle">{$txt_ajouter_obs}</a>  <br />
							<a href="#save_box"  class="boxy" title="Sauvegarde"  id="sauvegarder">{$txt_sauvegarder} !</a>   <br />
							<a href="{$application_path}modelisation/demo/{$etageSelectionne}">{$txt_demo} !</a><br /><br />
							<a href="{$application_path}modelisation/supprimer/{$etageSelectionne}" class="reinit" onclick="return confirm('{$txt_conf_reinit}');">{$txt_reinit} !</a>
						</div> 
						<br style="clear:both;">
			...
			<ul id="onglets">
			 ...
			 <div id="conteneurEtage">
					...
					<div id="rayons" style="height:750px;">
					...
					<div  id="scrolletage">
					 <div class="etage" id="{$arrayEtages[$cle]->getIdetage()}" style="position:relative;width:{$arrayEtages[$cle]->getlargeur()}px;height:{$arrayEtages[$cle]->gethauteur()}px;">		
						...
						{foreach from=$arrayObstacles key=myId item=i}  
							...
							{else}
								<div class="obstacle" id="{$i->getIdObstacle()}" libelle="{$i->getLibelle()}" style="position:absolute;top:{$i->getPosition_top()}px;left:{$i->getPosition_left()}px;width:{$i->getLargeur()}px;height:{$i->getHauteur()}px;">
							...
							{if {$i[0]->getPosition_top()} != "-1" && {$i[0]->getPosition_left()} != "-1"}
								...
								{/if}
									<div id="id_{$i[0]->getIdrayon()}" class="{$class}" sens="{$i[0]->getSens()}" type_ray="{$type}" largeur_ray="{$width}" hauteur_ray="{$height}"  top_deb="{$top}" left_deb="{$left}" style="top:{$top}px;left:{$left}px;width:{$width}px;height:{$height}px;{$border};">									
										<a href="#" class="rotation_rayon" onclick="return false;"   style="display:block;float:left;margin-right:2px; "><img src="{$application_path}images/rotate.png" style="width:12px;height:12px;" alt="R"/></a>	
										<a href="#" class="voir_info" onclick="return false;"  style="display:block;float:left;"><img src="{$application_path}images/info.png" style="width:12px;height:12px;" alt="I"/></a>
									</div>	
							{/if}
						{/foreach}
					</div>
